...
class HawaiPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('hawai.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hawai`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hawai WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hawai (date, title, link, content)"
